...
function create_results_arrays(N_its, epi_params_true)
  ...
  change_init_res = fill(-99., N_its, 36)
  # [is_accepted, log_α_ratio, post, post_prime] for
  # change_init_S_to_E, change_init_S_to_I, change_init_E_to_S, change_init_E_to_I,
  # change_init_I_to_S, change_init_I_to_E, change_init_p_env
  ...
  init_conds_update_tracker = fill(-99., N_its, 37)
  # change_init_S_to_E_track, change_init_S_to_I_track, change_init_E_to_S_track, change_init_E_to_I_track
  # change_init_I_to_S_track, change_init_I_to_E_track, change_init_p_env_track

  return(res, other_res, aug_res, tuning_res, update_tracker, change_init_res, init_conds_update_tracker)
end

function rename_results_arrays(res, other_res, aug_res, tuning_res, update_tracker, change_init_res, init_conds_update_tracker)

  res = DataFrame(res, :auto)
  rename!(res, [:β_c, :γ, :F, :ϵ, :ρ, :ρ_E])
  ...
  change_init_res = DataFrame(change_init_res, :auto)
  rename!(change_init_res, [:is_accepted_S_to_E, :log_α_ratio_S_to_E, :post_S_to_E, :post_prime_S_to_E, :reason_S_to_E,
                            :is_accepted_S_to_I, :log_α_ratio_S_to_I, :post_S_to_I, :post_prime_S_to_I, :reason_S_to_I,
                            :is_accepted_E_to_S, :log_α_ratio_E_to_S, :post_E_to_S, :post_prime_E_to_S, :reason_E_to_S,
                            :is_accepted_E_to_I, :log_α_ratio_E_to_I, :post_E_to_I, :post_prime_E_to_I, :reason_E_to_I,
                            :is_accepted_I_to_S, :log_α_ratio_I_to_S, :post_I_to_S, :post_prime_I_to_S, :reason_I_to_S,
                            :is_accepted_I_to_E, :log_α_ratio_I_to_E, :post_I_to_E, :post_prime_I_to_E, :reason_I_to_E,
                            :is_accepted_penv, :log_α_ratio_penv, :post_penv, :post_prime_penv, :reason_penv,
                            :sample])
  ...
  rename!(update_tracker, [:mSE_position, :mSE_t, :mSE_is_accepted, :mSE_reason, :mSE_Δt, :mSE_num_moved,
    ...
    init_conds_update_tracker = DataFrame(init_conds_update_tracker, :auto)
    rename!(init_conds_update_tracker,[:ciSE_position, :ciSE_t, :ciSE_is_accepted, :ciSE_reason, :ciSE_num_moved,
                                       ...
                                       :cipenv_position, :cipenv_t, :cipenv_is_accepted, :cipenv_reason, :cipenv_Δ,
                                                          :cirpenv_p_env_prev_before, :cipenv_p_env_prev_after])
...
function Initialise(DATA_res_and_track_cur, DATA_pers_and_parish_cur, epi_params_true, epi_params_dists, moves_record, dict_of_movements, f_to_p_structs::Vector{Farm_Parish_info})
  ...
  # Current values
  β_c_cur, γ_cur, F_cur, ϵ_cur, ρ_cur, ρ_E_cur = fill(missing, 6)

  epi_params_draw = fill(missing, 6)

  # True values
  β_c_true, γ_true, F_true, ϵ_true, ρ_true, ρ_E_true = epi_params_true

  # Prior distributions
  d_β_c, d_γ, d_F, d_ϵ, d_ρ, d_ρ_E = epi_params_dists

  # Set up LLH
  llh_array_init = zeros(size(DATA_res_and_track_cur[1], 1), 360, 8)
  p_env_llh_array_init = zeros(size(DATA_pers_and_parish_cur[2], 1), 360, 2)

  scope_init = Scope(1, 360, Vector(1:size(DATA_res_and_track_cur[1], 1)), Vector(1:8))
  ...
  while arb == true
    ...
    # Current draws
    epi_params_draw = [β_c_cur, γ_cur, F_cur, ϵ_cur, ρ_cur, ρ_E_cur]
...
function Blk_Adaptive_RWM_MCMC(;N_its, infer_block, data_aug_infer,
  ...
  ########################
    ...
    res, other_res, aug_res, tuning_res, update_tracker, change_init_res, init_conds_update_tracker = create_results_arrays(N_its, params_init)
  ...
  global_scope = Scope(1, 360, Vector(1:size(DATA_res_and_track_cur[1], 1)), Vector(1:8))
    ...
    while it <= N_its
      ...
      #######################################
        ...
        if infer_block[1] == true
          ...
          if it > 1
            fit!(covarM_inf, res[(it-1), [1,2,3,4]])
          end
          ...
          params_cur, llh_array_cur, p_env_llh_array_cur, DATA_res_and_track_cur, DATA_pers_and_parish_cur, mh_res_inf, mixture, λ_inf =
                    metropolis_hastings_step_params(N_its, res, other_res, it,
                                                    ...
                                                    n_tune, m_inf, λ_inf, 4, covarM_inf)
      ...
      #################################
        ...
        #################################
          ...
          mh_res_change_init_S_to_E::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
          change_init_S_to_E_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf]

          mh_res_change_init_S_to_I::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
          change_init_S_to_I_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf]

          mh_res_change_init_E_to_S::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
          change_init_E_to_S_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf]

          mh_res_change_init_E_to_I::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
          change_init_E_to_I_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf]

          mh_res_change_init_I_to_S::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
          change_init_I_to_S_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf]

          mh_res_change_init_I_to_E::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
          change_init_I_to_E_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf]

          if data_aug_infer[10] == true

            DATA_res_and_track_cur, DATA_pers_and_parish_cur, llh_array_cur, p_env_llh_array_cur, mh_res_change_init_S_to_E, change_init_S_to_E_track =
                                                    metropolis_hastings_step_aug(propose_change_init_S_to_E, generic_posterior_SE_dataaug,
                                                                                  params_cur, DATA_res_and_track_cur, DATA_pers_and_parish_cur,
                                                                                  llh_array_cur, p_env_llh_array_cur,
                                                                                  f_to_p_structs, ids_to_pos_dict,
                                                                                  moves_record, dict_of_movements)
          ...
          ############################
          ### Change Initial p_env ###
          ############################

            mh_res_change_init_penv::Vector{Float64} = [-Inf, -Inf, Inf, Inf, Inf]
            change_init_penv_track::Vector{Float64} = [-Inf, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf]
      ...
      ##########################
        ...
        update_tracker[it,:] = [move_SE_track ; move_EI_track ; AddRem_SE_track ; AddRem_EI_track ;
         ...
         # Record aug
         change_init_res[it,:] = [mh_res_change_init_S_to_E ; mh_res_change_init_S_to_I ; mh_res_change_init_E_to_S ; mh_res_change_init_E_to_I ;
                                  mh_res_change_init_I_to_S ; mh_res_change_init_I_to_E ; mh_res_change_init_penv ; it]

         # Record tracking
         init_conds_update_tracker[it,:] = [change_init_S_to_E_track ; change_init_S_to_I_track ; change_init_E_to_S_track ; change_init_E_to_I_track ;
                                            change_init_I_to_S_track ; change_init_I_to_E_track ; change_init_penv_track]
